	...
	useEffect(() => {
		async function load() {
			if (localStorage.personalizedDashboard) {
				...
				const filter = x =>
					x.default.includes(platform) &&
					x.default.includes(personGroup);
				setShownDashboardEntries(ALL_DASHBOARD_CARDS.filter(filter));
				setHiddenDashboardEntries(
					ALL_DASHBOARD_CARDS.filter(x => !filter(x))
				);
			}
				...
				<div className={styles.cardDeck}>
					<Modal
						...
						<Modal.Body ref={themeModalBody}>
							...
							<Form>
								{themes.map((availableTheme, i) => (
									<Button
										...
										variant={
											theme === availableTheme.style
												? "primary"
												: "secondary"
										}
										onClick={() =>
											changeTheme(availableTheme.style)
										}
										disabled={
											availableTheme.requiresToken &&
											unlockedThemes.indexOf(
												availableTheme.style
											) === -1
										}>
										{availableTheme.name}
									</Button>
							...
							<p>
								Um das <i>Hackerman</i>-Design freizuschalten,
								musst du mindestens vier Aufgaben unseres{" "}
								<a
									href={CTF_URL}
									target="_blank"
									rel="noreferrer">
								...
								lösen. Wenn du so weit bist, kannst du es{" "}
								<Link href="/become-hackerman">hier</Link>{" "}
								freischalten.
							</p>

							<h3 className={styles.themeHeader}>Dashboard</h3>
							<p>
								Hier kannst du die Reihenfolge der im Dashboard
								angezeigten Einträge verändern.
							</p>
							<ListGroup>
								{shownDashboardEntries.map((entry, i) => (
									<ListGroup.Item
										...
										<div
											className={styles.personalizeLabel}>
											{entry.label}
										</div>
										<div
											className={
												styles.personalizeButtons
											}>
											<Button
												variant="text"
												onClick={() =>
													moveDashboardEntry(i, -1)
												}>
												<FontAwesomeIcon
													title="Nach oben"
													icon={faChevronUp}
					...
					{shownDashboardEntries.map(entry =>
						entry.card(hideDashboardEntry)
					)}
				</div>
			</AppBody>
